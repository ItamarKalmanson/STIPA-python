...
def estimate_snr_per_band(ref_aligned, deg_aligned, fs, octave_bands):
    ...
    # Define the noise region using a fixed time window
    start_sample = int(0.5 * fs)
    end_sample = int(2.5 * fs)

    # Ensure the window is within the signal's bounds
    if end_sample > len(ref_aligned):
        print(
            f"Warning: The fixed noise window (up to 2.5s) exceeds the signal length ({len(ref_aligned) / fs:.2f}s). Adjusting window.")
    ...
    if not np.any(is_silent):
        print("Warning: Could not define a noise region. Check signal length and time window.")
        return [np.nan] * len(octave_bands), np.zeros_like(ref_aligned, dtype=bool)
    ...
    for band in octave_bands:
        ...
        if noise_power < 1e-20:
            ...
            if true_signal_power <= 0:
                snr_db = 0.0
            else:
                snr_db = 10 * np.log10(true_signal_power / noise_power)
...
def calculate_damage_attribution(mtf_measured, snr_per_band, octave_bands, mod_freqs):
    ...
    loss_from_noise = 1.0 - sti_noise_limited
    loss_from_reverb = 1.0 - sti_reverb_limited
    total_potential_loss = loss_from_noise + loss_from_reverb

    if total_potential_loss < 1e-6:
        noise_percent, reverb_percent = 0.0, 0.0
    else:
        noise_percent = (loss_from_noise / total_potential_loss) * 100
        reverb_percent = (loss_from_reverb / total_potential_loss) * 100

    return (noise_percent, reverb_percent), mtf_noise_only_matrix, mtf_reverb_only_matrix
...
def plot_spectrogram_with_noise_markers(signal, fs, is_silent, filename):
    """
    Generates a spectrogram of the signal and overlays markers for parts
    identified as noise.
    """
    ...
    if np.any(is_silent):
        ...
        for start_sample, end_sample in zip(starts, ends):
            ...
            label = 'Noise Region' if first_span else None
            ax.axvspan(start_time, end_time, color='red', alpha=0.3, label=label)
            first_span = False
...
def plot_ti_breakdown(ti_per_band, ti_mtf, ti_noise, octave_bands, filename):
    """
    Generates a bar chart breaking down the Transmission Index (TI) for each octave band.
    """
...
def generate_diagnostic_plots(
    ...
    """Generates a multi-panel figure for debugging and analysis."""
    noise_perc, reverb_perc = damage_contrib
    ...
    ax2.set_title("2. Estimated SNR per Octave Band", fontsize=14)
    ...
    ax3.axis('equal')
    ax3.set_title("3. STI Damage Source Attribution", fontsize=14)

    ax4 = axes[1, 1]
    band_to_plot_idx = 4  # 2000 Hz
...
def generate_report(sti_score, ti_per_band, snr_per_band, damage_contrib, octave_bands, filename):
    """Prints a clear, formatted report to the console."""
    noise_perc, reverb_perc = damage_contrib

    print("\n" + "=" * 70)
    print(f"📊 Advanced STI Analysis Report for: {os.path.basename(filename)}")
    print("=" * 70)
    ...
    print("\n--- Degradation Source Attribution ---")
    print(f"Estimated damage from NOISE:          {noise_perc:>5.1f}%")
    print(f"Estimated damage from REVERBERATION:  {reverb_perc:>5.1f}%")

    if noise_perc > 65:
        primary_issue = "The primary source of degradation is BACKGROUND NOISE."
    elif reverb_perc > 65:
        primary_issue = "The primary source of degradation is REVERBERATION (echo)."
    elif noise_perc > 40 and reverb_perc > 40:
        primary_issue = "Degradation is caused by a significant mix of both NOISE and REVERBERATION."
    elif noise_perc > reverb_perc:
        primary_issue = "Degradation is mainly from NOISE, with a smaller contribution from reverberation."
    else:
        primary_issue = "Degradation is mainly from REVERBERATION, with a smaller contribution from noise."
    print(f"\nConclusion: {primary_issue}")
    ...
    for i, band in enumerate(octave_bands):
        ...
        if np.isnan(snr_val):
            interp = "SNR couldn't be estimated"
        elif snr_val < 5:
            interp = "Very noisy"
        elif snr_val < 15:
            interp = "Noisy"
        ...
        if ti_val < 0.45:
            interp += ", Poor intelligibility"
        elif ti_val < 0.60:
            interp += ", Fair intelligibility"
        else:
            interp += ", Good intelligibility"

        snr_str = f"{snr_val:<10.1f}" if not np.isnan(snr_val) else "N/A"
        print(f"{f'{band} Hz':<15} | {snr_str} | {ti_val:<10.3f} | {interp}")

    print("\n--- Interpretation Guide ---")
    print("-> High 'Noise' damage corresponds to low SNR values in the table above.")
    print("-> High 'Reverberation' damage corresponds to low MTF values for high modulation")
    print("   frequencies (right side of the heatmap), even in clean octave bands.")
    print("=" * 70)
...
def analyze_sti(ref_file, deg_file, align_the_signals=True, show_plots=True):
    """
    Performs a full STI analysis on a signal, attributing damage to noise and reverb.
    """
    ...
    if fs_ref != fs_deg:
        target_fs = 44100
        print(
            f"Warning: Sample rates do not match (Ref: {fs_ref} Hz, Deg: {fs_deg} Hz). Resampling both to {target_fs} Hz.")
    ...
    if align_the_signals:
        ...
        ref_aligned = ref_sig[:min_len]
        deg_aligned = deg_sig[:min_len]
    ...
    damage_contrib, mtf_noise, mtf_reverb = calculate_damage_attribution(mtf_matrix, snr_per_band, octave_bands,
                                                                         mod_freqs)

    generate_report(sti_score, ti_per_band, snr_per_band, damage_contrib, octave_bands, deg_file)
    ...
    if show_plots:
        ...
        generate_diagnostic_plots(
            ref_aligned, deg_aligned, fs,
            snr_per_band, damage_contrib,
            mtf_matrix, mtf_noise, mtf_reverb,
            octave_bands, mod_freqs, deg_file
        )

    results = {
        "filepath": deg_file,
        "filename": os.path.basename(deg_file),
        "sti_score": sti_score,
        "noise_damage_percent": damage_contrib[0],
        "reverb_damage_percent": damage_contrib[1],
